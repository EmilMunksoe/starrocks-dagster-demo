...
def multi_catalog_analytics(context: AssetExecutionContext) -> None:
    ...
    with get_starrocks_connection("energy_trading") as (conn, cursor):
        ...
        for row in cursor.fetchall():
            context.log.info(
                f"  Decision {row[0]}: price=${row[1]:.2f}, trade={row[2]}, "
        ...
        context.log.info(
            "  1. default_catalog.energy_trading.trading_decisions (Native OLAP)"
        )
        context.log.info("  2. hive_catalog.raw_data.weather (Delta Lake via Hive)")
        context.log.info(
            "  3. postgres_catalog.public.DBS/TBLS (PostgreSQL Hive Metastore)"
        )
